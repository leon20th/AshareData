...
- Python 3.9+（依赖只设下限，`pip` 自动选择与当前解释器兼容的版本，不要求固定 Python 版本）；依赖清单见 `requirements.txt`（`setup.sh` 自动安装）：`pandas`、`pyarrow`、`polars`、`numpy`、`tqdm`、`selenium`、`baostock`、`openpyxl`、`pypinyin`（可选）、`torch`/`torchvision`（滑块定位模型）。
